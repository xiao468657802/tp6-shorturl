...
if ($_POST['type'] == 'toLong') {
    ...
    if ($_POST['kind'] == 'isgd') {
        ...
        //$short_url = get("https://is.gd/api.php?longurl=" . $long_url);    上面是坏的
    } elseif ($_POST['kind'] == 'unu') {
        $short_url = get("https://u.nu/api.php?action=shorturl&format=simple&url=" . $long_url);
        //$short_url = get("https://u.nu/api.php?action=shorturl&format=simple&url=" . $long_url);     上面是坏的
    } elseif ($_POST['kind'] == 'tinyurl') {
        $short_url = get("https://tinyurl.com/api-create.php?url=" .$long_url);
        //  这条域名失效
    } elseif ($_POST['kind'] == 'qaw') {
        $short_url = get("https://qaw.bio/api.php?signature=3d58c1088f&format=simple&action=shorturl&url=" .$long_url);
        //http://qaw.bio/api.php?signature=3d58c1088f&format=simple&action=shorturl&url=      如果API网站为开启ssl 则 写 http
    } elseif ($_POST['kind'] == 'vwlin') {
        $short_url = get("http://vwlin.cn/api.php?signature=3d58c1088f&format=simple&action=shorturl&url=" .$long_url);
        //http://qaw.bio/api.php?signature=3d58c1088f&format=simple&action=shorturl&url=      如果API网站为开启ssl 则 写 http
